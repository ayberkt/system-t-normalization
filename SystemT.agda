...
module SystemT where
  ...
  data _⊢_ (Γ : Ctx) : Type → Set where
    ...
    v   : {τ : Type} → τ ∈ Γ → Γ ⊢ τ
    lam : {τ1 τ2 : Type} → Γ ,, τ1 ⊢ τ2 → Γ ⊢ τ1 ⇒ τ2
    app : {τ1 τ2 : Type} → Γ ⊢ τ1 ⇒ τ2 → Γ ⊢ τ1 → Γ ⊢ τ2
  ...
  module RenamingAndSubstitution where
    ...
    extend⊇ : {Γ : Ctx} (Γ' : Ctx) {τ : Type} → Γ ⊇ Γ' → (Γ ,, τ) ⊇ Γ'
    extend⊇ [] ren = <>
    ...
    shift : {Γ : Ctx} {τ : Type} (Γ' : Ctx) (i : τ ∈ Γ) → τ ∈ (Γ' ++ Γ)
    shift [] i = i
    ...
    ⊇-single : {Γ : Ctx} {τ : Type} → (Γ ,, τ ⊇ Γ)
    ⊇-single = extend⊇ _ (⊇-id _)

    -- you can rename a term

    rename : {Γ Γ' : Ctx} {τ : Type} → Γ' ⊇ Γ → Γ ⊢ τ → Γ' ⊢ τ
    rename ρ c = c
    rename (_ , x') (v i0) = v x'
    ...
    addvar : {Γ Γ' : Ctx} {τ : Type} → Γ ⊢c Γ' → (Γ ,, τ) ⊢c (Γ' ,, τ)
    ...
    subst : {Γ Γ' : Ctx}{τ : Type} → Γ ⊢c Γ' → Γ' ⊢ τ  → Γ ⊢ τ
    subst θ c = c
    subst (θ , e) (v i0) = e
  ...
  module OpSem where
    -- step relation
    data _↦_ : {τ : Type} → [] ⊢ τ → [] ⊢ τ → Set where
      Step/app  :{τ1 τ2 : Type} {e e' : [] ⊢ τ1 ⇒ τ2} {e1 : [] ⊢ τ1}
             → e ↦ e'
             → (app e e1) ↦ (app e' e1)
      Step/β : {τ1 τ2 : Type} {e : [] ,, τ1 ⊢ τ2} {e1 : [] ⊢ τ1}
             → (app (lam e) e1) ↦ subst1 e1 e

    -- reflexive/transitive closure
    data _↦*_ : {τ : Type} → [] ⊢ τ → [] ⊢ τ → Set where
      Done : {τ : Type} {e : [] ⊢ τ} → e ↦* e
      Step : {τ : Type} {e1 e2 e3 : [] ⊢ τ}
           → e1 ↦ e2  →  e2 ↦* e3
           → e1 ↦* e3
  open OpSem
  ...
  module VeryWeakNormalization where
    ...
    head-expand : (τ : Type) {e e' : [] ⊢ τ} → e ↦ e' → WN τ e' → WN τ e
    head-expand = {!!}
  ...
  module WeakNormalization where
    ...
    {- TASK 6b -}
    corollary2 : {τ1 τ2 : Type} (e : [] ⊢ τ1 ⇒ τ2) → Σ \(e' : _) → e ↦* (lam e')
    corollary2 e = {!!}
